...
namespace Odasoft.XBOL.ClientAPI.Controllers
{
    ...
    public class SeatManagementController(
        ...
        ILogger<SeatManagementController>? logger = null
    ) : ControllerBase
    {
        ...
        public async Task<ActionResult<BookingResult>> BookSeatsAsync([FromBody] BookSeatsBody request)
        {
            ...
            logger?.LogInformation(
                "Resolved seat booking client identity: verified client {VerifiedClientId}, forwarded contact client {ClientContactId}, bundle {BundleId}, event schedule {EventScheduleId}, payment link {IsPaymentLink}",
                verifiedClientId,
                request.ClientContact.Id,
                request.BundleId,
                request.EventScheduleId,
                request.IsPaymentLink);
