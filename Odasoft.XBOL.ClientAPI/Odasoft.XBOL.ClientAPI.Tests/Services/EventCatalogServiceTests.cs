...
public sealed class EventCatalogServiceTests
{
    ...
    [Fact]
    public async Task GetBundleBannerAsync_UsesBundleMediaForSeasonPassBundles()
    {
        await using var database = await TestDatabase.CreateAsync();
        var now = DateTimeOffset.UtcNow;
        ...
        await database.Context.SaveChangesAsync();
        database.Context.Media.Add(CreateMedia(ClientSaleType.Bundle, bundle.Id, "https://cdn.example.test/bundle-banner.jpg"));
        database.Context.Media.Add(CreateMedia(ClientSaleType.SeasonPass, bundle.Id, "https://cdn.example.test/season-banner.jpg", order: -1));
        await database.Context.SaveChangesAsync();
        var sut = CreateBundleService(database.Context);

        var result = await sut.GetBundleBannerAsync(includeMedia: true);

        result.Should().NotBeNull();
        result!.BannerImageUrl.Should().Be("https://cdn.example.test/bundle-banner.jpg");
        result.Media.Should().NotBeNull();
        result.Media!.Banner.Should().NotBeNull();
        result.Media.Banner!.Url.Should().Be("https://cdn.example.test/bundle-banner.jpg");
    }

    [Fact]
    public async Task GetItemsAsync_UsesBundleMediaForSeasonPassBundleLegacyImageFields()
    {
        await using var database = await TestDatabase.CreateAsync();
        var now = DateTimeOffset.UtcNow;
        var bundle = CreateBundle(
        ...
        database.Context.Bundles.Add(bundle);
        await database.Context.SaveChangesAsync();
        database.Context.Media.Add(CreateMedia(ClientSaleType.Bundle, bundle.Id, "https://cdn.example.test/catalog-bundle-banner.jpg"));
        database.Context.Media.Add(CreateMedia(ClientSaleType.SeasonPass, bundle.Id, "https://cdn.example.test/catalog-season-banner.jpg", order: -1));
        await database.Context.SaveChangesAsync();
        var sut = new EventCatalogService(database.Context);
        ...
        item.BannerImageUrl.Should().Be("https://cdn.example.test/catalog-bundle-banner.jpg");
        item.PosterImageUrl.Should().Be("https://cdn.example.test/catalog-bundle-banner.jpg");
    }
    ...
    [Fact]
    public async Task GetBundleSeasonByIdAsync_UsesBundleMediaForSeasonPassBundles()
    {
        await using var database = await TestDatabase.CreateAsync();
        var now = DateTimeOffset.UtcNow;
        var previousBundle = CreateBundle(9, BundleType.SeasonPass, "Previous Season", now, now.AddMonths(-8), now.AddMonths(-2));
        ...
        database.Context.Bundles.AddRange(previousBundle, bundle);
        await database.Context.SaveChangesAsync();
        database.Context.Media.Add(CreateMedia(ClientSaleType.Bundle, bundle.Id, "https://cdn.example.test/booking-bundle-banner.jpg"));
        database.Context.Media.Add(CreateMedia(ClientSaleType.SeasonPass, bundle.Id, "https://cdn.example.test/booking-season-banner.jpg", order: -1));
        await database.Context.SaveChangesAsync();
        var sut = CreateBookingService(database.Context);

        var result = await sut.GetBundleSeasonByIdAsync(bundle.Id, clientId: null, includeMedia: true);

        result.Should().NotBeNull();
        result!.BannerImageUrl.Should().Be("https://cdn.example.test/booking-bundle-banner.jpg");
        result.Media.Should().NotBeNull();
        result.Media!.Banner.Should().NotBeNull();
        result.Media.Banner!.Url.Should().Be("https://cdn.example.test/booking-bundle-banner.jpg");
    }
