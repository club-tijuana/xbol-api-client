...
namespace Odasoft.XBOL.Data.Repositories
{
    public class OrderRepository(XBOLDbContext dbContext) : BaseRepository<Order>(dbContext)
    {
        ...
        public async Task<MyEventsDiagnosticsProjection> GetMyEventsDiagnosticsAsync(
            OrderType orderType,
            long idClient)
        {
            var paidOrdersByType = DbContext.Set<Order>()
                .Where(o =>
                    o.Status == OrderStatus.Paid
                    && o.ClientId == idClient
                    && o.OrderType == orderType);

            var qualifyingTicketOrders = paidOrdersByType
                .Where(o => o.Tickets.Any(t =>
                    (t.OriginalClientId == idClient || t.CurrentClientId == idClient)
                    && t.EventSchedule.Event.Status == EventStatus.Published
                    && t.EventSchedule.Event.DeletedAt == null));

            var bundlePassItemOrders = paidOrdersByType
                .Where(o => o.Items.Any(i => i.ItemType == ItemType.BundlePass));

            var ownedBundlePasses = DbContext.Set<BundlePass>()
                .Where(bp => bp.ClientId == idClient);

            var activeOwnedBundlePasses = ownedBundlePasses
                .Where(bp => bp.Status == BundlePassStatus.Active);

            var publishedActiveBundlePassItemOrders = bundlePassItemOrders
                .Where(o => o.Items.Any(i =>
                    i.ItemType == ItemType.BundlePass
                    && activeOwnedBundlePasses.Any(bp =>
                        bp.Id == i.ItemReferenceId
                        && bp.Bundle.Status == EventStatus.Published
                        && bp.Bundle.DeletedAt == null)));

            return new MyEventsDiagnosticsProjection
            {
                PaidOrdersByType = await paidOrdersByType.CountAsync(),
                QualifyingTicketOrders = await qualifyingTicketOrders.CountAsync(),
                BundlePassItemOrders = await bundlePassItemOrders.CountAsync(),
                OwnedBundlePasses = await ownedBundlePasses.CountAsync(),
                ActiveOwnedBundlePasses = await activeOwnedBundlePasses.CountAsync(),
                PublishedActiveBundlePassItemOrders = await publishedActiveBundlePassItemOrders.CountAsync()
            };
        }
